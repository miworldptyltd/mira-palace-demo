...
<!-- R020: Tailwind v4 compiled build.
     ...
     runtime JIT). Now: a pre-compiled site.css (~15-30 KB minified) built
     by tools/tailwindcss.exe from scripts/tailwind/input.css. The @theme
     block that used to live inline as `tailwind.config = {{...}}` is now
     CSS-first in scripts/tailwind/input.css. -->
<script>
      ...
      <div class="bg-mira-900/85 backdrop-blur supports-[backdrop-filter]:bg-mira-900/75 text-white border-b border-white/5">
        <div class="max-w-7xl mx-auto px-5 lg:px-8 flex items-center justify-between gap-4 h-16 xl:h-[68px]">
          <a href="{root}index.html" class="flex items-center gap-2 shrink-0">
            <!-- R029.5: single canonical SVG monogram — gold circle + dark M.
                 Shared with the footer and the favicon so the brand mark is
                 identical wherever it appears. Inline <img src=".svg"> keeps
                 CSS out of it and lets browsers cache one file. -->
            <img src="{root}assets/img/mp-monogram.svg" alt="Mira Palace logo" class="w-9 h-9" width="36" height="36" fetchpriority="high" />
        ...
        <div id="nav-search-pop" class="hidden absolute right-4 lg:right-8 top-full mt-2 w-[360px] max-w-[92vw] bg-white rounded-md shadow-lux border border-mira-200 overflow-hidden">
          <div class="flex items-center gap-2 px-3 py-2 border-b border-mira-100">
            ...
            <input id="nav-search-input" type="search" placeholder="Search the site — rooms, spa, dining…" data-i18n-placeholder="nav.search.placeholder" class="flex-1 bg-transparent border-none outline-hidden text-sm text-mira-900 placeholder:text-mira-500" autocomplete="off" />
        ...
        <div id="mobile-menu" class="xl:hidden hidden border-t border-white/5 max-h-[80vh] overflow-y-auto">
          ...
          <div class="px-4 py-3 border-b border-white/10 bg-mira-900/40">
            <div class="flex items-center gap-2 bg-white/8 border border-white/15 rounded px-3 py-2">
              ...
              <input id="mnu-search-input" type="search" placeholder="Search the site…" data-i18n-placeholder="nav.search.placeholder_mobile" class="flex-1 bg-transparent border-none outline-hidden text-sm text-white placeholder:text-white/50" autocomplete="off" />
            </div>
      ...
      <div class="max-w-7xl mx-auto px-5 lg:px-8 py-14 grid grid-cols-1 md:grid-cols-4 gap-10">
        <div class="md:col-span-1">
          <div class="flex items-center gap-2">
            <img src="{root}assets/img/mp-monogram.svg" alt="Mira Palace logo" class="w-9 h-9" width="36" height="36" />
...
def _hotel_jsonld(page_url: str) -> str:
    ...
    blob = {
        ...
        "logo": m["base_url"].rstrip("/") + "/assets/img/mp-monogram.svg",
        "image": m["og_image"],
